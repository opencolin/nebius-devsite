...
const STATIC_ROUTES: Array<{path: string; changefreq: string; priority: number}> = [
  ...
  // /ecosystem = mixed Apps + Integrations grid. Higher priority than
  // the sibling /apps and /integrations pages since it's the umbrella
  // view a first-time visitor probably wants.
  {path: '/ecosystem', changefreq: 'daily', priority: 0.9},
  ...
  // Product landing pages — bespoke .tsx pages mirroring tenki.cloud.
  ...
  // Tavily web-search onboarding page — linked from the Products menu.
